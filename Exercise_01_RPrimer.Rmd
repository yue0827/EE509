...
2.	Why are we multiplying the results of the atan and atan2 functions by 180/pi? 
    Because we need to transfer the angle from radian to degree.
3.	How is the atan2 function different from the atan function? 
    The atan2(y,x) function contains two arguements and returns the angle between the x-axis and the vector from the origin to (x,y) while the atan function compute arc-tangent of the only one variable.
4.	What is the difference between log and log10?
    log is natrual logarithm and log 10 is log base 10.
...
8.	Describe the difference in output between sum and cumsum.
    The sum function returns the sum of all values in a vector while cumsum returns the cumulative sum of values in a vector.
...
dat2 = read.table("my_frogs.txt",sep="|",header=TRUE)
...
superheros <- c("WonderWomen","BlackWidow","Thor","Ironman")
...
23.	Using the frog data
  ...
  reg = lm(tadpoles ~ frogs,data=dat[dat$color=="red",])
  summary(reg)
  b.	Make a scatter plot of this data that includes the regression line
  dat1=subset(dat,color=="red")
  plot(dat1$frogs,dat1$tadpoles)
  abline(reg)
  ...
  reg2 = lm(tadpoles ~       
        frogs+spots+color+frogs*spots+frogs*color+color*spots,data=dat)
  summary(reg2)
```
...
my.plot = function(dat1,reg1){
plot(dat1$frogs,dat1$tadpoles,
     cex=1.5,  			# increase the symbol size
     col=as.character(dat1$color),	# change the symbol color by name
     pch=dat1$spots+1,			# change the symbol (by number)
     cex.axis=1.3,			# increase the font size on the axis 
     xlab="Frog Density",		# label the x axis
     ylab="Tadpole Density",		# label the y axis
     cex.lab=1.3,			# increase the axis label font size
...
dat = subset(dat,tadpoles>=5)
reg = lm(tadpoles ~ frogs,data=dat)
my.plot(dat,reg)

...
```{R}
AIC.null <- 2*lnL[1]+2*3
AIC.tmt <- 2*lnL[2] + 2*lnL[3] +2*6
dAIC <- AIC.null - AIC.tmt
## Difference of AIC between the null model and the alternate model is 44.15 which is very large. It's strongly supported that the latter model is better. And the result of the LRT shows the p-value is very small, which means the alternative hypothesis that the treatment (CO2) has a significant effect on maturation and fecundity can be accepted.
...
abline(v=a0.ci,lty=2)
abline(v=a0[2])
plot(density(mle[,2],width=1),type='l')
abline(v=b0.ci,lty=2)
abline(v=b0[2])
plot(density(mle[,3],width=0.5),type='l')
abline(v=b1.ci,lty=2)
abline(v=b1[2])

std = c(sd(mle[,1])/sqrt(100),sd(mle[,2])/sqrt(100),sd(mle[,3])/sqrt(100))
tbl <- data.frame(para = colnames(mle),CI2.5 = matrix(c(a0.ci[1],b0.ci[1],b1.ci[1]),nrow=3),
                  CI97.5 = matrix(c(a0.ci[2],b0.ci[2],b1.ci[2]),nrow=3),
                  std = matrix(std,nrow=3))
tbl

## If nboot is set to be small, the MLE para will have relatively low density. 
```
...
```{R}
conf.full   <- matrix(NA,nrow=nboot,ncol=npred)	
pred.full <- matrix(NA,nrow=nboot,ncol=npred)	
for(i in 1:nboot){
  conf.full[i,] <- mle[i,1]*dseq^2.*pnorm(dseq,mle[i,2],mle[i,3]) 
  pred.full[i,] <- rpois(npred,conf.full[i,])
}
ci.full <- apply(conf.full,2,quantile,c(0.025,0.5,0.975))
pi.full <- apply(pred.full,2,quantile,c(0.025,0.975)) 
plot(dia.t ,ncone.t,xlab='dia',ylab = 'ncone',main = "full fecundity model")
lines(dseq,ci.full[2,],col=2,lwd=3)
lines(dseq,ci.full[1,],col=2,lty=2,lwd=3)	## 95% CI
lines(dseq,ci.full[3,],col=2,lty=2,lwd=3)
lines(dseq,pi.full[1,],col=3,lty=2,lwd=3)	## 95% PI
lines(dseq,pi.full[2,],col=3,lty=2,lwd=3)

## fecundity plot
cones = b$c00 > 0
plot(b$diam,cones,col = b$tmt,ylim=c(0,1),main = "maturation model")
lines(dseq,ci.mat[2,],col=2,lwd=3)   ## median model
lines(dseq,ci.mat[1,],col=2,lty=2,lwd=3)	## 95% CI
lines(dseq,ci.mat[3,],col=2,lty=2,lwd=3)
lines(dseq,pi.mat[1,],col=3,lty=2,lwd=3)	## 95% PI
...
## In both plots, PIs are wider than CIs because predictive intervals include both parameter and data uncertainties. 
```
...
```{R}
dia.e   <- b$diam[tmt=="CO2"]   ## tree diameters
cones.e <- (b$c00 > 0)         ## whether cones are present
cones.e <- cones[tmt=="CO2"]
ncone.e <- b$c00[tmt=="CO2"] 
mle2   <- matrix(NA,nrow=nboot,ncol=3)
for(i in 1:nboot){
  if(i%%100 == 0) print(i)  					## progress indicator
  samp <- sample(length(dia.e),replace=T)		##Sample row indices 
  out.boot2 <- optim(param,likfit,method="L-BFGS-B",	## fit model to sample
                    lower=c(0.001,10,1),upper=c(1,30,30),dia=dia.e[samp],ncone=ncone.e[samp])
  mle2[i,] <- out.boot2$par					## store parameters
}
```

```{r}
colnames(mle2) = c("a0","b0","b1")
a0.ci2 <- quantile(mle2[,1],c(0.025,0.975))
b0.ci2 <- quantile(mle2[,2],c(0.025,0.975))
b1.ci2 <- quantile(mle2[,3],c(0.025,0.975))
pairs(mle2)

## density plots
plot(density(mle2[,1],width=0.005),type='l',xlim=c(0,0.05),ylim=c(0,120))
abline(v=a0.ci2,lty=2)
abline(v=a0[3])
plot(density(mle2[,2],width=1),type='l')
abline(v=b0.ci2,lty=2)
abline(v=b0[3])
plot(density(mle2[,3],width=0.5),type='l')
abline(v=b1.ci2,lty=2)
abline(v=b1[3])

std2 = c(sd(mle2[,1])/sqrt(100),sd(mle2[,2])/sqrt(100),sd(mle2[,3])/sqrt(100))
tbl2 <- data.frame(para = colnames(mle2),
                   CI2.5 = matrix(c(a0.ci2[1],b0.ci2[1],b1.ci2[1]),nrow=3),
                  CI97.5 = matrix(c(a0.ci2[2],b0.ci2[2],b1.ci2[2]),nrow=3),
                  std = matrix(std,nrow=3))
tbl2
```
...
```{R}
conf.full2   <- matrix(NA,nrow=nboot,ncol=npred)	
pred.full2 <- matrix(NA,nrow=nboot,ncol=npred)	
for(i in 1:nboot){
  conf.full2[i,] <- mle2[i,1]*dseq^2.*pnorm(dseq,mle2[i,2],mle2[i,3]) 
  pred.full2[i,] <- rpois(npred,conf.full2[i,])
}
ci.full2 <- apply(conf.full2,2,quantile,c(0.025,0.5,0.975))
pi.full2 <- apply(pred.full2,2,quantile,c(0.025,0.975)) 
plot(b$diam ,b$c00,xlab='dia',col = b$tmt,ylab = 'fecundity',main = "Ambient vs Elevated")
lines(dseq,ci.full[2,],col=1,lwd=3)
lines(dseq,ci.full[1,],col=1,lwd=2)	## 95% CI
lines(dseq,ci.full[3,],col=1,lwd=2)
lines(dseq,pi.full[1,],col=1,lty=2,lwd=2)	## 95% PI
lines(dseq,pi.full[2,],col=1,lty=2,lwd=2)
lines(dseq,ci.full2[2,],col=2,lwd=3)
lines(dseq,ci.full2[1,],col=2,lwd=2)	## 95% CI
lines(dseq,ci.full2[3,],col=2,lwd=2)
lines(dseq,pi.full2[1,],col=2,lty=2,lwd=2)	## 95% PI
lines(dseq,pi.full2[2,],col=2,lty=2,lwd=2)
```

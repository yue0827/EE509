...
plot(dia,cones)
xseq = seq(0,30,length=200)
lines(xseq,pnorm(xseq,b0,b1),type='l',col=2)
...
```{R}
likmat = function(beta,x,y){
  theta = pnorm(x,beta[1],beta[2])
  -sum(dbinom(y,1,theta,log=TRUE)) ## negative log likelihood
}
b0 = 30
b1 = 3
param = c(b0,b1)
out.mat <- nlm(likmat,param,x=dia,y=cones)
out.mat

b0 = out.mat$estimate[1]
b1 = out.mat$estimate[2]
plot(dia,cones)
xseq = seq(0,30,length=200)
lines(xseq,pnorm(xseq,b0,b1),type='l',col=2)
```
...
```{r}
param  <- c(0.008,21,7)        ## initial guess (update this based on the first two models!)
...
dseq = seq(0,30,length=100)       ## seq of x values for drawing curve  
plot(dia,ncone,xlab='dia',ylab = 'Fecundity',main = "dia vs ncone")
lines(dseq,a0*dseq^2,col=2,lwd=2)  ## add fecundity relationship
lines(dseq,a0*dseq^2*pnorm(dseq,b0,b1),col=3,lwd=2)
legend('topleft',lty=1,legend = c('g(x)','g(x)theta(x)'),col = c(2,3))

plot(dia,cones,xlab='dia',ylab = 'Maturation',main = 'dia vs cones')
lines(dseq,pnorm(dseq,b0,b1),col=2) 
...
out.elev
para = data.frame(a0,b0,b1,lnL,n)
para
```
...
dseq = seq(0,30,length=100)       ## seq of x values for drawing curve  
plot(dseq,a0[1]*dseq^2,type = 'l',col=1,xlab='dia',ylab = 'g(x)',main = "dia vs ncone")  
lines(dseq,a0[2]*dseq^2,col=2)
lines(dseq,a0[3]*dseq^2,col=3)
legend('topleft',lty=1,legend = c('full','ambient','elevated'),col = c(1,2,3))

plot(dseq,a0[1]*dseq^2*pnorm(dseq,b0[1],b1[1]),type = 'l',col=1,xlab='dia',ylab = 'g(x)theta(x)',main = "dia vs ncone")  
lines(dseq,a0[2]*dseq^2*pnorm(dseq,b0[2],b1[2]),col=2)
lines(dseq,a0[3]*dseq^2*pnorm(dseq,b0[3],b1[3]),col=3)
legend('topleft',lty=1,legend = c('full','ambient','elevated'),col = c(1,2,3))

plot(dseq,pnorm(dseq,b0[1],b1[1]),col=1,type='l',xlab='dia',ylab = 'Maturation',main = 'dia vs cones')  
lines(dseq,pnorm(dseq,b0[2],b1[2]),col=2)
lines(dseq,pnorm(dseq,b0[3],b1[3]),col=3)
legend('topleft',lty=1,legend = c('full','ambient','elevated'),col = c(1,2,3))

```
```
1. Elavated CO2 will increase the fecundity, because from the second plot, the line with elevated CO2 exceed both the full and the ambient scenarios in fecundity.
2. The third plot tells us the elevated CO2 will increase the maturation rate, and further increase the fecundity.
```

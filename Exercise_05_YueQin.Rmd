...
mean.b1 <- mean(out[,2])
mean.b2 <- mean(out[,3])
lines(x1,mean.b1+mean.b2*x1,col=2)
```
* Describe and explain the parameter covariances that you observe in the pairs plot and parameter correlation matrix.
```{R}
pairs(out)	
cor(out)
# The pairs plots show that the variance and regression parameters are not linerly correalated which scatter spreadly but the two regression parameters are highly correlated with each other, which in the plot represented by the scatters distributed linerly. It is consistent with the parameter correlation matrix that the correlation coefficient between b1 and b2 is very close to 1 and that between s and b1/b2 are fairly small.
```
...
# The means and uncertanties(standard deviations) of b1 and b2 are similar between two methods, but the residual varies alot. The variance(residual) in lm is larger than that in Bayesian regression model across all the quantiles. 
```
...
## specify priors
data2$b0 <- as.vector(c(0,0,0,0))
data2$Vb <- solve(diag(10000,4))
data2$s1 <- 0.1
data2$s2 <- 0.1
...
## running jags
j.model2 <- jags.model(file = textConnection(multivariate_regression),
                       data = data2,
                       inits = inits2,
                       n.chains = 3)
...
## evaluating MCMC outputs
plot(jags.out2)
gelman.diag(jags.out2) ## convergence diagnostics
GBR2 <- gelman.plot(jags.out2)
burnin = 500
jags.burn2 <- window(jags.out2,start=burnin)
plot(jags.burn2)
acfplot(jags.burn2)
